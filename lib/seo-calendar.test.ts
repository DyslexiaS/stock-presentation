...
describe('calendar SEO keywords', () => {
  test('puts the primary phrase in the title and supporting phrases in the description', () => {
    ...
    expect(String(meta.title)).not.toContain('法說會時間表、近期法說會一覽表')
    expect(String(meta.description)).toContain('法說會行事曆')
    expect(String(meta.description)).toContain('法說會時間表')
  ...
  test('JSON-LD collection name is 法說會行事曆', () => {
    ...
    expect(page.description).toContain('法說會行事曆')
  })
})
