...
NVIDIA's high-density AI rack is a **liquid-cooled** rack. This hub does two things: it reads what NVIDIA has actually published about that loop, then maps the Taiwan-listed companies that have to build against it. Power is a separate topic — see the [NVIDIA 800 VDC hub](/en/topics/nvidia-800v). Every company below has an English memo page with briefings of its earnings calls.
