...
目前三個主題：NVIDIA 800 VDC、AI 機櫃液冷、SiC/GaN。34 家公司、50 篇季度 memo，每家有獨立 company profile。Memo 用 reader-first 結構（At a glance / numbers / 800V read-through / Q&A），公司背景不重複寫進每季。**Management Q&A** 是 `**Q:**` / `**A:**` 一問一答，對照中文摘要直翻、留原數字與但書，不縮成短評。鴻海已補到 2026Q2。液冷主題頁在 `/en/topics/liquid-cooling`，掛奇鋐、雙鴻、健策、建準、富世達、尼得科超眾。SiC/GaN 主題頁在 `/en/topics/sic-gan`，掛漢磊、嘉晶、朋程、捷敏（化合物主線 dual-tag，不把全部 800V 筆記灌進去）。`/llms.txt` 與 `/en-sitemap.xml` 由 loader 動態列出全部英文 URL，新增 memo 不必手改。
