...
└── topics/                           # 主題 hub（nvidia-800v, liquid-cooling, sic-gan, rack-odm）
...
| `/en/[company]` | Lite-On earnings call English | 37 |
| `/en/[company]/[quarter]` | Lite-On Q2 2026 earnings call | 53 |
...
```
/sitemap-index.xml
  ├── /en-sitemap.xml                 # /en + 名錄 + 全部 calls + 主題 + 37 家公司 + 53 篇 memo
...
- [x] **英文 memo 專區 `/en`** — Markdown SSG，NVIDIA 800V、液冷、SiC/GaN 與 rack ODM 為前四個主題
...
2. 主題用 `tags`（例如 `nvidia-800v`、`liquid-cooling`、`sic-gan`、`rack-odm`），不要寫進網址。冷卻、化合物或組裝主線可雙掛；電源機櫃筆記不要灌進液冷、SiC/GaN 或把機箱廠灌進 rack ODM。
