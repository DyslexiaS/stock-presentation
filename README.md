...
└── topics/                           # 主題 hub（nvidia-800v, liquid-cooling, sic-gan）
...
| `/en/[company]` | Lite-On earnings call English | 35 |
| `/en/[company]/[quarter]` | Lite-On Q2 2026 earnings call | 51 |
...
```
/sitemap-index.xml
  ├── /en-sitemap.xml                 # /en + 名錄 + 全部 calls + 主題 + 35 家公司 + 51 篇 memo
...
- [x] **英文 memo 專區 `/en`** — Markdown SSG，NVIDIA 800V、液冷與 SiC/GaN 為前三個主題
...
目前三個主題：NVIDIA 800 VDC、AI 機櫃液冷、SiC/GaN。35 家公司、51 篇季度 memo，每家有獨立 company profile。Memo 用 reader-first 結構（At a glance / numbers / 800V read-through / Q&A），公司背景不重複寫進每季。**Management Q&A** 是 `**Q:**` / `**A:**` 一問一答，對照中文摘要直翻、留原數字與但書，不縮成短評。鴻海已補到 2026Q2。液冷主題頁在 `/en/topics/liquid-cooling`，掛奇鋐、雙鴻、健策、建準、富世達、尼得科超眾。SiC/GaN 主題頁在 `/en/topics/sic-gan`，掛環球晶、漢磊、嘉晶、朋程、捷敏（化合物主線 dual-tag，不把全部 800V 筆記灌進去）。漢磊沒有比 2025-09-17 更新的完成轉錄。`/llms.txt` 與 `/en-sitemap.xml` 由 loader 動態列出全部英文 URL，新增 memo 不必手改。
...
2. 主題用 `tags`（例如 `nvidia-800v`、`liquid-cooling`、`sic-gan`），不要寫進網址。冷卻主線或化合物主線可雙掛；電源機櫃筆記不要灌進液冷或 SiC/GaN 主題。
